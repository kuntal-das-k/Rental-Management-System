...
export const ProductDetail: React.FC = () => {
  ...
  const { city, pincode, isDetecting, error: locationError, detectGPSLocation, setLocation } = useLocationStore();
  ...
  const [customPincode, setCustomPincode] = useState<string>(pincode);
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-12 py-8 flex-1 w-full space-y-8 z-10 ">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            {/* Card 1.5: GPS Location & Delivery Availability Widget */}
            <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-bold tracking-wider text-slate-800 uppercase">
                  ...
                  <span>Check Delivery & GPS Location</span>
                </div>
                <button
                  onClick={() => detectGPSLocation()}
                  disabled={isDetecting}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-[11px] font-bold transition-all shadow-xs disabled:opacity-50"
                >
                  {isDetecting ? (
                    <>
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#F3F4F6] border-0 rounded-xl px-3.5 py-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-slate-900 focus:outline-none"
                  />
                  <button
                    onClick={() => setLocation(customPincode || 'Mumbai', customPincode || '400001')}
                    className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-900 text-xs font-bold rounded-xl transition-colors"
                  >
                    Check
                  </button>
                </div>
                ...
                {locationError && (
                  <p className="text-[11px] text-amber-600 font-semibold">{locationError}</p>
                )}

                {/* Real-time Availability Alert Box */}
                <div className="p-3 bg-emerald-50 border border-emerald-200/80 rounded-xl space-y-1.5">
                  <div className="flex items-center gap-2 text-emerald-900 font-bold text-xs">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>Product Available for Rental in {city} ({pincode})</span>
                  </div>
                  <p className="text-[11px] text-emerald-700 font-medium pl-6">
                    ⚡ Express Delivery available by Tomorrow 10:00 AM from {product.vendor?.company_name || 'local vendor hub'}.
                  </p>
                </div>
              </div>
            </div>
